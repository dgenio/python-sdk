...
async def test_proxy_message_transformation(create_streams):
    """Test that message transformation callbacks work."""
    client_streams, server_streams, (client_read_writer, _), (_, server_write_reader) = create_streams()

    try:
        transformed = []

        async def transform_client_message(msg: SessionMessage) -> SessionMessage | None:
            """Transform messages from client."""
            transformed.append("client")
            # Modify the message ID
            if hasattr(msg.message.root, "id"):
                original_id = msg.message.root.id
...
async def test_proxy_message_dropping(create_streams):
    """Test that returning None from transform callback drops the message."""
    client_streams, server_streams, (client_read_writer, _), (_, server_write_reader) = create_streams()

    try:

        async def drop_message(msg: SessionMessage) -> SessionMessage | None:
            """Drop all messages."""
            return None
...
async def test_proxy_transform_error_handling(create_streams):
    """Test that errors in transform functions are caught."""
    client_streams, server_streams, (client_read_writer, _), (_, server_write_reader) = create_streams()
    ...
    try:
        ...
        async def failing_transform(msg: SessionMessage) -> SessionMessage | None:
        ...
        async with mcp_proxy(
            client_streams, server_streams, onerror=error_handler, on_client_message=failing_transform
        ):
            await client_read_writer.send(message)
